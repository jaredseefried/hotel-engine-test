...
const RepositoryItem = (props) => {
  ...
  return (
    <>
      <li className="repo-item row" onClick={() => setModalShow(true)}>
        <Col className='repo-item-col' sm={12} md={6} lg={2}>
          <img src={props.image} alt="..." className='rounded github-avatar' />
        </Col>
        <Col className='repo-item-col' sm={12} md={6} lg={2}>
          <h6 className="user-name repo-info">{props.name}</h6>
        </Col>
        <Col className='repo-item-col' sm={12} md={6} lg={3}>
          <p className="repo-info">{props.description}</p>
        </Col>
